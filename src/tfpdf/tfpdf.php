...
namespace minga\framework\tfpdf;

use minga\framework\tfpdf\font\unifont\TTFontFile;
...
class tFPDF
{
	...
	function getFontStream($font, &$codeToGlyph, &$maxUni, &$ttfontsize)
	{
		Profiling::BeginTimer();
		$subset = $font['subset'];
		unset($subset[0]);
		// se fija si se van del rango 0 - 255
		$min = 255;
		$max = 0;
		...
		if ($min >= 1 && $max <= 255)
			// usa el ASCII
		{
			$stackFile = $font['ttffile'] . ".ascii";
			$stackFileMeta = $font['ttffile'] . ".json";
			if (file_exists($stackFile) == false)
			{
				$ttf = new TTFontFile();
				$subset2 = range(1, 255);
				$ttfontstream2 = $ttf->makeSubset($font['ttffile'], $subset2);
				$ttfontsize = strlen($ttfontstream2);
				$ret = gzcompress($ttfontstream2);
				$codeToGlyph = $ttf->codeToGlyph;
				$maxUni = $ttf->maxUni;
				IO::WriteAllText($stackFile, $ret);
				$metadata = array("maxUni" => $maxUni,
					"ttfontsize" => $ttfontsize,
					"codeToGlyph" => $codeToGlyph);
				IO::WriteJson($stackFileMeta, $metadata);
			}
			else
			{
				$ret = IO::ReadAllText($stackFile);
				$metadata = IO::ReadJson($stackFileMeta);
				$ttfontsize = $metadata['ttfontsize'];
				$codeToGlyph = $metadata['codeToGlyph'];
				$maxUni = $metadata['maxUni'];
			}
		}
		else
		{
	...
	function _putfonts()
	{
		...
		foreach($this->fonts as $k => $font)
		{
			...
			else if ($type == 'TTF')
			{
				...
				$fontstream = $this->getFontStream($font, $codeToGlyph, $maxUni, $ttfontsize);
				unset($codeToGlyph[0]);
				...
				$this->_putTTfontwidths($font, $maxUni);
