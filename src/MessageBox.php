...
class MessageBox
{
	...
	public static function ThrowMessage($message, $action = '', $title = 'Atención', $caption = 'Continuar')
	{
		...
		{
			echo "<html><body onload=\"parent.ThrowMessage('" . Str::EscapeJavascript( $message) . "');\"></body></html>";
			exit();
		}
	...
	public static function ThrowInternalServerError($exception = null)
	{
		...
		if (Context::Settings()->Debug()->debug)
		{
			...
			{
				$log .= $i .'=>'.$t['file'].'('.$t['line']."): " .$t['class'].'::'.$t['function']."(). <br>";
			}
			if ($exception != null)
	...
	public static function ThrowFileNotFound($extraInfo = "")
	{
		...
		if (Context::Settings()->Debug()->debug)
		{
			...
			foreach ($trace as $i=>$t)
			{
				$log .= $i .' => <a href="repath://'.$t['file'].'@'.$t['line'].'">'.$t['file'].' ('.$t['line'].')</a>: ' .$t['class'].'::'.$t['function'].'(). <br>';
			}
			$log .= "</p>";
